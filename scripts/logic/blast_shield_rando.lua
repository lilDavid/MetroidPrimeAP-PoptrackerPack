local function can_open_door_color(area, color)
    if color == "AnyBeam" then
        return true
    end
    if color == "Missile" then
        return can_missile() or has("BlastShieldRando")
    end
    local location = Tracker:FindObjectForCode("@doors/" .. area .. "/" .. color)
    if not location then
        if ENABLE_DEBUG_LOG then
            print("can_open_door_color: unrecognized color " .. tostring(color) .. " for " .. tostring(area))
        end
        return false
    end
...
local function create_blast_shield_item(area, source, destination, forward_type, reverse_type)
    ...
    if blast_shield_items[item_code] ~= nil then
        return blast_shield_items[item_code]
    end
    if blast_shield_items[reverse_code] ~= nil then
        return blast_shield_items[item_code]
    end

    if reverse_type == nil then
        reverse_type = forward_type
    end
    ...
    item.ProvidesCodeFunc = function(self, code)
        if not self:CanProvideCodeFunc(code) then
            return false
        end
        ...
        if code == reverse_code then
            return can_open_door_color(area, reverse_type)
        end
...
function can_open(area, source, destination)
    ...
    if missile_door_list == nil or mixitup_door_list == nil then
        if ENABLE_DEBUG_LOG then
            print("can_open: unrecognized area " .. area)
        end
        return true
    end
    ...
    if missile_door == nil then
        missile_door = missile_door_list[destination .. "|" .. source]
    end
    ...
    if mixitup_door ~= nil then
        ...
        if mixitup_door ~= nil then
            if mixitup_door[2] == nil then
                mixitup_door = mixitup_door[1]
            else
                mixitup_door = mixitup_door[2]
            end
        end
    end
